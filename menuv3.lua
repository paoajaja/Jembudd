--[[
    ZONE XD MOBILE V6 - LOCKED EDITION
    PASSWORD: Apis123
    FIX JUMP & FLY SYSTEM
...
-- ==================== PASSWORD SYSTEM ====================
local PASSWORD = "Apis123"
local isUnlocked = false
local passwordAttempts = 0
local waitingForPassword = true
...
local function ShowLockedMessage()
    ...
    SendAutoReply("/me ╔════════════════════════════════════════╗")
    wait(0.3)
    SendAutoReply("/me ║     🔒 ZONE XD - TERKUNCI 🔒          ║")
    wait(0.3)
    SendAutoReply("/me ╠════════════════════════════════════════╣")
    wait(0.3)
    ...
    SendAutoReply("/me ║                                        ║")
    wait(0.3)
    SendAutoReply("/me ║  KETIK: /pw [password]                ║")
    wait(0.3)
    SendAutoReply("/me ║                                        ║")
    wait(0.3)
    SendAutoReply("/me ║  CONTOH: /pw Apis123                  ║")
    wait(0.3)
    SendAutoReply("/me ╚════════════════════════════════════════╝")
...
local function CheckPassword(input)
    if string.lower(input) == string.lower(PASSWORD) then
        isUnlocked = true
        ...
        wait(0.2)
        SendAutoReply("/me ║     🔓 ZONE XD UNLOCKED 🔓             ║")
        wait(0.2)
        SendAutoReply("/me ╠════════════════════════════════════════╣")
        wait(0.2)
        ...
        Notify("🔓 UNLOCKED", "Zone XD aktif! Selamat menggunakan!", 4)
        return true
    else
        passwordAttempts = passwordAttempts + 1
        SendAutoReply("/me ⛔ PASSWORD SALAH! Percobaan ke-" .. passwordAttempts .. " ⛔")
        Notify("⛔ SALAH", "Password salah! Coba lagi!", 3)
        return false
    end
end
...
local function ShowProfile()
    ...
    SendAutoReply("/me ╠════════════════════════════════════════════════╣")
    wait(0.1)
    SendAutoReply("/me ║ 💀 ZONE XD V6 LOCKED EDITION 💀                ║")
    wait(0.1)
...
local function StartFly()
    ...
    SendAutoReply("/me 🕊️ FLY MODE AKTIF! Geser joystick untuk terbang 🕊️")
...
local function ShowMenu()
    ...
    wait(0.1)
    SendAutoReply("/me ╚════════════════════════════════════════════════╝")
    Notify("🎮 ZONE XD V6", "Menu ditampilkan di chat!", 5)
end
...
local function ProcessCommand(msg)
    ...
    -- Password command (always available)
    if string.match(lower, "^/pw") then
        local pw = string.match(msg, "^/pw%s+(.+)$")
        if pw then
            CheckPassword(pw)
        else
            SendAutoReply("/me 🛡️ Format: /pw [password] 🛡️")
        end
        return true
    end
    
    -- If not unlocked, only show locked message for other commands
    if not isUnlocked then
        ShowLockedMessage()
        return true
    end
...
-- ==================== INIT ====================
local function Init()
    Notify("🔒 ZONE XD V6", "Terkunci! Masukkan password...", 3)
    task.wait(1)
    SetupChatListener()
    ShowLockedMessage()
end
