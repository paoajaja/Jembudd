-- ZONE XD V8.1 - FIXED FOR ALL EXECUTORS
...
local Settings = {
    ...
    TrollSpam = false, TrollFreeze = false, TrollFlung = false, TrollBlind = false,
    TrollEarrape = false, TrollSpin = false, TrollInvert = false,
    TrollChatSpam = false, TrollFire = false, TrollRagdoll = false,
    TrollTarget = nil, SpamMessage = "ZONE XD V8 - BOCIL KONTOL", SpamDelay = 0.5
}
...
local function SetSpeed(v)
    ...
    Notify("⚡ SPEED", "Kecepatan: "..v, 2) end
end
...
local function ToggleTrollFreeze()
    ...
    if Settings.TrollFreeze then
        ...
        if Settings.TrollTarget then freezePlayer(Settings.TrollTarget) end
...
local function ToggleTrollSpin()
    ...
    if Settings.TrollSpin then
        ...
        TrollSpinConnection = RunService.RenderStepped:Connect(function()
            ...
            if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                local root = target.Character.HumanoidRootPart
                root.Orientation = Vector3.new(root.Orientation.X, root.Orientation.Y + 45, root.Orientation.Z)
            end
        end)
...
local function TrollStop()
    Settings.TrollSpam = false; Settings.TrollFreeze = false; Settings.TrollFlung = false
    Settings.TrollBlind = false; Settings.TrollEarrape = false; Settings.TrollSpin = false
    Settings.TrollInvert = false; Settings.TrollChatSpam = false; Settings.TrollFire = false
...
local function ResetAll()
    ...
    Settings = {
        ...
        TrollSpin = false, TrollInvert = false, TrollChatSpam = false, TrollFire = false, TrollRagdoll = false,
        OriginalTransparency = {}, TrollTarget = nil, SpamMessage = "ZONE XD V8 - BOCIL KONTOL", SpamDelay = 0.5
    }
    if AutoClickConnection then AutoClickConnection:Disconnect() end
    if AimbotConnection then AimbotConnection:Disconnect() end
    if InfiniteJumpConnection then InfiniteJumpConnection:Disconnect() end
...
local function ShowMenu()
    ...
    wait(0.05)
    SendChat("/me ╚══════════════════════════════════════════════════════════════╝")
    Notify("🎮 ZONE XD V8", "Menu ditampilkan di chat!", 7)
end
